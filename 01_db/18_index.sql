-- index
-- 사전(클러스터링, 고유) 또는 용어 검색(넌클러스트링, 비고유)
...
-- Cardinality(카디널리티): phone_code에 들어있는 값들이 중독되지 않는 갯수(종류)

-- 인덱스를 태워서(활용해서) 검색하면 속도가 빠르다.(logN 시간으로 속도가 빠름)
...
-- 5) 복합속성일 경우 (컬럼 두 개 이상)는 카디널리티가 높은(중복이 적은)컬럼을 먼저 사용
