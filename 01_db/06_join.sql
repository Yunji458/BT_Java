...
-- 1) '메뉴명'과 '카테고리'명이 각각 어떤 테이블에 존재하나?(두 개 이상이면)
-- 2) tbl_menu와 tbl_category에서 확인을 하고 서로 관계를 맺었는지 확인
-- 3) join으로 해결 가능함을 확인

-- 메뉴를 기준으로 카테고리도 함꼐 보는 개념
SELECT 
       *
  FROM tbl_menu 
...
-- 별칭을 활용한 조인 사용
-- 테이블에 별칭 추가 시에는 as를 붙여도 안붙여도 된다.(통일감있게)
SELECT 
       a.menu_name AS '메뉴명'          
     , a.menu_price
     , b.category_name
...
SELECT
       *
  FROM tbl_menu
 CROSS JOIN tbl_category;  -- 메뉴 테이블, 카테고리 테이블 행x행
