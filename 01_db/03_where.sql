...
-- WHERE orderable_status != 'N';    -- !와 <>는 '같지 않다'를 의미
 WHERE orderable_status <> 'N'tbl_category;
 
 -- -------------------------------------------
 -- Q) '기타' 카테고리에 해당하지 않는 메뉴를 조화하시오.
...
-- 10000원 보다 초과하거나 5000원 이하인 메뉴 조회(OR)
...
--  OR category_code = 8
--  OR category_code = 10;
 WHERE category_code IN (5,8,10);    -- =와 or이 반복될 경우 IN연산자로 대체할 수 있다.
