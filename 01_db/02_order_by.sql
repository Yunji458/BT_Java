...
SELECT
     ...
     , menu_price
  FROM tbl_menu
-- ORDER BY menu_code DESC;                  -- Order By절은 해석 할 때 가장 마지막에 하자.
...
-- ---------------------------------------
-- 1. 반드시 코드는 아끼지말고 변경해 볼 것!
-- 2. 에러를 되려 띄워볼 것!
  
-- ---------------------------------------
-- 주문 불가능한 메뉴부터 보기
SELECT * FROM tbl_menu;                      -- 테이블의 데이터를 토대로 확인
DESC tbl_menu;                               -- DESC라는 명령어로 빠르게 인사이트 열기
-- DESCRIBE tbl_menu;
...
SELECT 
  ...
  FROM tbl_menu
 ORDER BY 주문가능상태;                      -- 별칭이 인지되고 나서 order by 되기 떄문
 
 -- --------------------------------------
 -- null값(비어있는 컬럼값)에 대한 정렬
 SELECT * FROM tbl_category;
 
 -- 1) 오름차순 시: null이 먼저 사용
 SELECT 
       *
   FROM tbl_category
  ORDER BY ref_category_code ASC;
 
 -- 2) 내림차순 시: null이 나중에 사용
 SELECT 
       *
   FROM tbl_category
  ORDER BY ref_category_code DESC;

 -- 3) 오름차순 시: null이 나중에 사용
 SELECT 
       *
   FROM tbl_category
  ORDER BY -ref_category_code DESC;   -- '-'는 반대의 의미이고 DESC를 썼지만 Null 이 뒤에 나오는 오름차순이 된다.
