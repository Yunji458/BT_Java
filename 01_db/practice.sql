...
SELECT 
  ...
  FROM employee A 
  left JOIN DEPARTMENT B ON (A.DEPT_CODE = B.DEPT_ID)
WHERE A.EMP_NAME LIKE '%동%'; -- left join과 join 차이 확인
...
-- 10. 연봉(보너스 포함)가 3000만원 이상인 직원(사번, 직원명, 부서명)
SELECT
       a.emp_id AS '사번'
     , a.emp_name AS '직원명'
     , b.dept_title AS '부서명'
--      , a.salary * (1 + nvl(a.bonus, 0)) * 12
  FROM employee a
  LEFT JOIN department b ON a.dept_code = b.dept_id
 WHERE a.salary * (1 + nvl(a.bonus, 0)) * 12 >= 30000000;
  
DESC employee;
DESC department;
  
-- 11. 급여 평균이 300만원 이상인 부서에서 일하는 직원(사번, 직원명, 부서명)
-- 급여 평균이 300만원 이상인 부유한 부서(서브쿼리)
SELECT
       dept_code
  FROM employee a
 GROUP by dept_code
HAVING AVG(a.salary * (1 + NVL(a.bonus, 0))) >= 3000000;

SELECT
       a.emp_id AS '사번'
     , a.emp_name AS '직원명'
     , b.dept_title AS '부서명'
--      , a.salary * (1 + nvl(a.bonus, 0))
  FROM employee a
  JOIN department b ON a.dept_code = b.dept_id
